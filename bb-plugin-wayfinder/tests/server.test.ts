...
describe("Wayfinder server integration", () => {
  it("registers bounded RPC, artifact routes, and an agent tool", async () => {
    ...
    expect(harness.inspection.registrations.agentTools.map((tool) => tool.name)).toEqual(expect.arrayContaining(["wayfinder_start", "wayfinder_record_start", "wayfinder_record_stop", "wayfinder_screenshot", "wayfinder_windows", "wayfinder_window_center", "wayfinder_desktop_click", "wayfinder_desktop_scroll"]));
    expect(harness.inspection.registrations.cli?.name).toBe("wayfinder");
    expect(harness.inspection.registrations.cli?.commands.map((command) => command.name)).toEqual(["record", "screenshot", "windows", "window", "click", "scroll", "doctor", "setup"]);
