...
describe("Computer panel", () => {
  ...
  it("allows control without an active run and disconnects immediately when hidden", async () => {
    ...
    const desktop = slot.getByLabelText("Live computer; click, drag, type, paste, or scroll") as HTMLImageElement;
    ...
    await waitFor(() => expect(slot.inspection.rpcCalls).toContainEqual({ method: "computer.control.input", input: { hostId: "host_thread", runId: null, clientId: expect.any(String), input: { kind: "click", x: 320, y: 180, button: "right" } } }));
    fireEvent.pointerDown(desktop, { pointerId: 1, button: 0, clientX: 100, clientY: 100 });
    fireEvent.pointerMove(desktop, { pointerId: 1, buttons: 1, clientX: 200, clientY: 200 });
    fireEvent.pointerUp(desktop, { pointerId: 1, button: 0, clientX: 200, clientY: 200 });
    await waitFor(() => expect(slot.inspection.rpcCalls.some((call) => call.method === "computer.control.input" && (call.input as { input?: { kind?: string } }).input?.kind === "drag")).toBe(true));
