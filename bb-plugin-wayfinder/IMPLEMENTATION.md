...
Wayfinder now treats the operating system's logged-in desktop as an external prerequisite. It never installs a desktop environment, creates desktop shortcuts, or substitutes a browser window for the computer. The plugin owns its private Cua Driver process, reviewed bounded capability manifest, capture/input requests, and private evidence pipeline. Linux and Windows use embedded mode. macOS uses Cua Driver's stable standalone TCC identity so the Screen Recording and Accessibility grants created by `cua-driver permissions grant` match the process performing capture.
