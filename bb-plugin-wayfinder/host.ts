...
async function execute(run: RunRecord): Promise<void> {
  ...
  try {
    ...
    const providerMode = run.route.decisionProvider?.provider ?? "fixture";
    const provider = providerMode === "fixture" ? new FixtureProvider() : providerMode === "jev"
      ? new JevDecisionProvider({ endpoint: run.route.decisionProvider?.endpoint ?? process.env.WAYFINDER_TYPESAFE_ENDPOINT ?? "https://api.typesafe.ai/v1/systemone", model: run.route.decisionProvider?.model ?? process.env.WAYFINDER_TYPESAFE_MODEL ?? "", apiKey: process.env.WAYFINDER_TYPESAFE_API_KEY ?? "", maxCalls: run.route.limits.maxProviderCalls, maxApproxTokens: run.route.limits.maxProviderTokens, maxRetries: run.route.limits.maxRequestRetries })
      : new OpenRouterDecisionProvider({ endpoint: run.route.decisionProvider?.endpoint ?? process.env.WAYFINDER_OPENROUTER_ENDPOINT, model: run.route.decisionProvider?.model ?? process.env.WAYFINDER_OPENROUTER_MODEL ?? "", apiKey: process.env.WAYFINDER_OPENROUTER_API_KEY ?? "" });
    ...
    const current = runs.get(run.runId)!; runs.set(run.runId, update(current, { state: result.state, activeController: true, finishedAt: Date.now(), error: result.error, cleanup: { state: "pending", message: null, completedAt: null }, checkpoints: result.checkpoints.map((checkpoint) => artifact ? { ...checkpoint, evidenceArtifactIds: [artifact.artifactId] } : checkpoint) }));
  } catch (error) {
    ...
    runs.set(run.runId, update(current, { state: signal.aborted ? "cancelled" : "blocked", activeController: true, finishedAt: Date.now(), error: failure, cleanup: { state: "pending", message: null, completedAt: null } }));
