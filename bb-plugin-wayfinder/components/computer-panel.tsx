...
export function ComputerPanel({ threadId, params }: PluginThreadPanelProps) {
  ...
  return <div ref={panelRoot} className="flex h-full min-h-0 flex-col bg-sidebar text-sidebar-foreground">
    <header className="flex h-12 shrink-0 items-center gap-2 border-b px-3">
      <button type="button" aria-label="All computers" onClick={() => selectMachine(null)} className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"><Icon name="ArrowLeft" className="size-4" aria-hidden="true" /></button>
      <Icon name="Laptop" className="size-4 text-muted-foreground" aria-hidden="true" />
      <span className="min-w-0 flex-1 truncate text-sm font-medium">{hostName}</span>
    </header>
    <main className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden p-3">
      <div className="group relative flex h-full min-h-0 w-full items-center justify-center overflow-hidden rounded-md border border-border bg-black text-white" style={{ aspectRatio: `${desktopSize.width} / ${desktopSize.height}` }}>
