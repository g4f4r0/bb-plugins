...
it("offers a responsive-mode toolbar toggle and compact viewport controls", () => {
  ...
  expect(viewerHtml).toContain(
    "responsiveToggle.disabled=!responsiveAvailable||responsiveStaging;",
  );
  expect(viewerHtml).toContain("responsiveLoadingIcon");
  expect(viewerHtml).toContain("Switching viewport…");
  ...
  expect(viewerHtml).toContain("prepareViewportFrame(width,height)");
  expect(viewerHtml).toContain("videoAvailable&&!enabled");
